...
public class MovieAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        // 이미지 세팅 잠시 보류
        String imageLinke = movieVOList.get(position).getImage();
        if(!imageLinke.isEmpty()) {
            // imageLink에 있는 이미지를 다운로드하여
            // mHolder.m_image ImageView에 표시하라
            Picasso.get().load(imageLinke).into(mHolder.m_image);

            // Glide.with(mHolder.itemView.getContext())
            //        .load(imageLinke).into(mHolder.m_image);
        }
    }
